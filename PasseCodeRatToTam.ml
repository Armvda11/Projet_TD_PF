...
let rec analyse_code_instruction i =
  match i with
  | AstPlacement.Declaration (info, e) | AstPlacement.DeclarationStatic (info, e) ->
    let (_, t, d, reg) = info_var info in
...
and analyse_code_bloc (li, _) = 
  let liste_codes = List.map analyse_code_instruction li in
